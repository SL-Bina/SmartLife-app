...
import {
  ...
  pickText,
  toRecord,
} from '../../resident/resident-data-utils';
...
const normalizeNotification = (
  ...
  return {
    id,
    title: pickText(value.title, `Bildiris #${id}`),
...
export default function DashboardNotificationsScreen() {
    ...
    async (targetPage: number, append: boolean) => {
      ...
      try {
        ...
        if (loadError instanceof Error && loadError.message) {
          ...
          setError('Bildirisler yuklenmedi');
        }
      } finally {
        if (append) {
  ...
  const onMarkAllRead = React.useCallback(async () => {
    try {
      ...
      // Keep local state as read to preserve responsiveness.
    } finally {
  ...
  return (
    <AppPageLayout
      ...
      notificationText={
        unreadCount > 0 ? `${unreadCount} oxunmamis bildiris` : 'Yeni bildiris yoxdur'
      }
    >
      <View style={[styles.hero, isDark ? styles.heroDark : styles.heroLight]}>
        ...
        <View style={styles.heroTextWrap}>
          ...
          <Text style={[styles.heroSubtitle, isDark ? styles.heroSubtitleDark : styles.heroSubtitleLight]}>
            Dashboard ucun real endpoint ile bildiris siyahisi
          </Text>
        </View>
