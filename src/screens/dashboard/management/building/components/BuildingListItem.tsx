...
export function BuildingListItem({
    ...
    const status: BuildingStatus =
        statusRaw === 'inactive' || statusRaw === '0' ? 'inactive' : 'active';

    const address = asText(item.address).trim() || asText(pickMeta(item).address).trim() || '-';
    const complexId = asText(item.complex_id).trim() || '-';
    const secondary = getBuildingSecondary(item);
    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    <Text style={styles.contactText}>Unvan: {address}</Text>
                    <Text style={styles.contactText}>Complex ID: {complexId}</Text>
                </View>
            </Pressable>
        </Animated.View>
    );
