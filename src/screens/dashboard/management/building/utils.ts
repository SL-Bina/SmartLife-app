...
export const getBuildingName = (item: EntityItem): string => {
  ...
  return id === null ? 'Adsiz Bina' : `Bina #${id}`;
};

export const getBuildingSecondary = (item: EntityItem): string => {
  const meta = pickMeta(item);
  const candidates = [item.address, meta.address, item.complex_id];
...
export const buildPayload = (form: BuildingFormState): Record<string, unknown> => {
  const parsedComplexId = Number(form.complexId.trim());

  return {
    name: form.name.trim(),
    complex_id: Number.isFinite(parsedComplexId) ? parsedComplexId : form.complexId.trim(),
