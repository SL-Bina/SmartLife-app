...
import { BuildingViewScreen } from './components/BuildingViewScreen';
...
export default function ManagementBuildingViewRouteScreen() {
    ...
    React.useCallback(() => {
      ...
      void (async () => {
        try {
          ...
          if (isMounted) {
            Alert.alert('Xeta', toErrorMessage(loadError, 'Bina detallari yuklenmedi'));
          }
        } finally {
          if (isMounted) {
            setLoading(false);
  ...
  const onDelete = React.useCallback((target: EntityItem) => {
    ...
    if (id === null) {
      Alert.alert('Xeta', 'Bina ID tapilmadi');
      return;
    }

    Alert.alert('Silme tesdiqi', 'Secilen Bina silinsin?', [
        ...
        onPress: () => {
          void (async () => {
            try {
              ...
              Alert.alert('Xeta', toErrorMessage(deleteError, 'Bina silinmedi'));
            } finally {
              setSubmitting(false);
            }
          })();
    ...
    >
      {loading ? (
        ...
        <BuildingViewScreen
          ...
          onEdit={entry => {
            navigation.push('ManagementBuildingForm', { editingItem: entry });
          }}
          onDelete={onDelete}
        />
      ) : (
