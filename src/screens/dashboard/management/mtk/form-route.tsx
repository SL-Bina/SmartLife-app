...
import { MtkFormScreen } from './components/MtkFormScreen';
import { mtkStyles as styles } from './styles';
import { EMPTY_FORM, EntityItem, MtkFormState } from './types';
...
const mapItemToForm = (item: EntityItem): MtkFormState => {
...
export default function ManagementMtkFormRouteScreen() {
  ...
  const [formState, setFormState] = React.useState<MtkFormState>(
  ...
  const onSubmit = React.useCallback(async () => {
    ...
    if (name.length === 0) {
      Alert.alert('Xeta', 'MTK adi daxil edilmelidir');
      return;
    }
    ...
    try {
      ...
      if (editingItem && editingId !== null) {
        await mtkAPI.update(editingId, payload);
      } else {
        await mtkAPI.add(payload);
      }

      navigation.goBack();
    } catch (submitError) {
      Alert.alert('Xeta', toErrorMessage(submitError, 'MTK yadda saxlanmadi'));
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    ...
    >
      <MtkFormScreen
        ...
        onChange={(patch: Partial<MtkFormState>) => {
