...
export function BlockListCard({
  ...
  const buildingId = asText(item.building_id).trim() || '-';
  const secondary = getBlockSecondary(item);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Text style={styles.contactText}>Bina ID: {buildingId}</Text>
          <Text numberOfLines={1} style={styles.contactText}>Tesvir: {secondary}</Text>
        </View>
      </Pressable>
    </Animated.View>
  );
